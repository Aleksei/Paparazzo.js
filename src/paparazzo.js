		...
		*/
		Paparazzo.prototype.handleServerResponse = function(chunk) {
			...
			if ( boundary_index !== -1 ) {
				...
				typeMatches = remaining.match(/Content-Type:\s*image\/jpeg\s*/i);
				matches = remaining.match(/Content-Length:\s*(\d+)\s*/i);

				if ((matches != null) && matches.length > 1) {
					newImageBeginning = Math.max( remaining.indexOf(matches[0]) + matches[0].length, typeMatches ? remaining.indexOf(typeMatches[0]) + typeMatches[0].length : 0);
